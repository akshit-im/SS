...
for /f "tokens=5" %%a in ('netstat -aon ^| find ":9096" ^| find "LISTENING"') do taskkill /f /pid %%a
for /f "tokens=5" %%a in ('netstat -aon ^| find ":9097" ^| find "LISTENING"') do taskkill /f /pid %%a
for /f "tokens=5" %%a in ('netstat -aon ^| find ":9098" ^| find "LISTENING"') do taskkill /f /pid %%a
